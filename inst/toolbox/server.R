...
function(input, output, session) {
  ...
  untarFiles <- function(path_to_tar, startDate, endDate, timestep, tar_flag = 1) {
    ...
    return(
      list(infile =  file.path(ordpath, infile),
           claas_flag = claas_flag, filelist = filelist)
    )
  }
    ...
    if (!file_info$has_lon_lat) {
      if (claas_flag) {
        ...
        if (grid_info == 5 && (is.null(globalAuxFilePath()) || !file.exists(globalAuxFilePath())) ) {
          showModal(modalDialog(
            h4("Your data seems to require an auxiliar file. You can either upload a local auxiliar file or download it drom the public DWD page."),
            br(),
        ...
        if (visualizeVariables()$plot_dim == 2) {
          ...
          output$lon_visualize <- renderUI({
            sliderInput("slider1",
                        ...
                        value = c(max(round(visualizeVariables()$min_lon), -180), min(round(visualizeVariables()$max_lon), 180)))
          })
          ...
          output$lat_visualize <- renderUI({
            sliderInput("slider2",
                        ...
                        value = c(max(round(visualizeVariables()$min_lat), -90), min(round(visualizeVariables()$max_lat), 90)))
          })
          ...
          shinyjs::hide("statistics_1d")
          shinyjs::show("statistics_2d")
          ...
          shinyjs::hide("statistics_2d")
          shinyjs::show("statistics_1d")
  ...
  observeEvent({
    ...
    old_height <- imageheight()
    old_width  <- imagewidth()
    ...
    # still trigger if no changes (e.g. sliding both ends of slider simultaneously)
    if (old_height == imageheight() && old_width == imagewidth()) {
  ...
  output$y_visualize <- renderUI({
    ...
    sliderInput("slidery",
                label = "Y-Range",
                min = round(visualizeDataMin() - (0.25 * visualizeDataMax())),
                max = round(visualizeDataMax() + (0.25 * visualizeDataMax())),
                value = c(visualizeDataMin(), visualizeDataMax()))
  })
  ...
  render_plot <- function(plot_rinstat) {
    ...
    if (input$proj == "rect") {
      # Adjusting Color dependent on user input
      if (input$checkGroup == 1) {
        col <- fields::tim.colors(input$num_brk)
      } else if (input$checkGroup == 2) {
        col <- grDevices::heat.colors(input$num_brk)
      } else if (input$checkGroup == 3) {
        col <- grDevices::terrain.colors(input$num_brk)
      } else if (input$checkGroup == 4) {
        col <- grDevices::colorRampPalette(c("darkblue", "white", "darkred"))
        col <- col(input$num_brk)
      } else if (input$checkGroup == 5) {
        col <- grDevices::colorRampPalette(c("darkred", "white", "darkblue"))
        col <- col(input$num_brk)
      } else if (input$checkGroup == 6) {
        col <- grDevices::rainbow(input$num_brk)
      } else if (input$checkGroup == 7) {
        col <- grDevices::cm.colors(input$num_brk)
      } else if (input$checkGroup == 8) {
        col <- grDevices::colorRampPalette(c("black",
                                            "#3a0303",
                                            "#640000",
                                            "#981800",
                                            "#ca4b00",
    ...
    if (input$proj == "ortho") {
      ...
      if (input$checkGroup == 1) {
        pcol <- fields::tim.colors(input$num_brk)
      }
      if (input$checkGroup == 2) {
        pcol <- grDevices::heat.colors(input$num_brk)
      }
      if (input$checkGroup == 3) {
        pcol <- grDevices::terrain.colors(input$num_brk)
      }
      if (input$checkGroup == 4) {
        co <- grDevices::colorRampPalette(c("darkblue", "white", "darkred"))
        pcol <- co(input$num_brk)
      }
      if (input$checkGroup == 5) {
        co <- grDevices::colorRampPalette(c("darkred", "white", "darkblue"))
        pcol <- co(input$num_brk)
      }
      if (input$checkGroup == 6) {
        pcol <- grDevices::rainbow(input$num_brk)
      }
      if (input$checkGroup == 7) {
        pcol <- grDevices::cm.colors(input$num_brk)
      }
      if (input$checkGroup == 8) {
        co <- grDevices::colorRampPalette(
          c(
            "black",
            "#3a0303",
            "#640000",
  ...
  getPlot_2d <- reactive({
    # Required triggers
    req(imagewidth())             # image width  is updated on lon/lat_bound update
    req(imageheight())            # image height is updated on lat/lat_bound update
    req(input$proj)               # projection
    req(visualizeDataMax())       # max data (not sure why want to trigger this?)
    req(visualizeDataTimestep())  # when data at timestep changes
    ...
    c(name_loc_vec(),    # new location
      input$checkGroup,  # colorbar
      input$text1,       # title
      input$text2,       # sub-title
      input$text3,       # scale caption
      input$int,         # country borders
      input$region,      # for region plot
      input$xort,        # center lat
      input$yort,        # center lon
      input$rort,        # rotation
      instat_path(),     # instat file path change
      input$num_tick,    # number ticks
      input$num_rmin,    # min val
      input$num_rmax,    # max val
      input$num_brk,     # number breaks
      lat_lon_trigger()  # changes to lat, lon, bounds
      )
  ...
  output$myImage_1d <- renderImage({
    ...
    req(input$slidery)
    req(input$integer)
    req(input$checkGroup_type)
    c(input$trend)
    c(input$analyze_timeseries)
    req(input$ticknumber)
  ...
  output$about <- renderPrint({
    ...
    cat("Your NetCDF data have to follow CF conventions and the data","\n")
    cat("have to be on a rectangular grid with longitude, latitude","\n")
    cat("and time dimension.","\n")
    cat("This version ('Don`t Panic') was tested with the cmsaf","\n")
    cat("R-package in version 2.0.0.","\n")
    cat("\n")
    cat("Suggestions for improvements and praise for the developers","\n")
    cat("can be send to contact.cmsaf@dwd.de.","\n")
    cat("\n")
    cat("                              - Steffen Kothe - 2019-07-25 -","\n")
    cat("\n")
    cat("\n")
  })
